<x-app-layout mainClass="container grow my-8 mx-auto py-8 shadow shadow-black/25
    ...
    <article class="-mx-6">
        <header
            ...
            <div class="pt-2 pr-1">
                <h2><i class="fa-solid fa-graduation-cap pr-1 text-white"></i> {{ __('Unit List') }} </h2>
                {{-- <form action="{{ route('units.index') }}" method="GET" class="pr-2 pb-2 text-black"> --}}
                    {{-- @csrf
...
{{--            <div class="grow"></div>--}}
        ...
        <div class="flex flex-col flex-wrap my-4 mt-8">
            {{-- @if ($search != null || $filter != null)
                <p class="px-5">
                    @if ($search != null)
                        Search results for: "<b>{{ $search }}</b>" [{{ $units->total() ." of ". $unitCount }} course(s) found]
                    @else
                        Filtered for the package: "<b>{{ $filter->national_code .': '. $filter->title }}</b>" [{{ $units->total() ." of ". $unitsCount }} course(s) found]
                    @endif
                    <x-primary-link-button href="{{ route('units.index') }}" class="float-end">
                        Show All Units
                    </x-primary-link-button>
                </p>
            @endif --}}
